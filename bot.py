...
class HealthServer:
    ...
    async def dashboard_endpoint(self, request):
        ...
        try:
            ...
            return web.Response(text=f"Dashboard loading error: {e}", status=500)
    ...
    async def stats_endpoint(self, request):
        """Statistics endpoint with API key status"""
        stats = get_database_stats()
        bot_username = get_bot_username()
        
        api_id_connected = bool(config.API_ID and config.API_ID != 1234567)
        api_hash_connected = bool(config.API_HASH and config.API_HASH != "your_api_hash_here")
        bot_token_connected = bool(config.BOT_TOKEN and config.BOT_TOKEN != "your_bot_token_here")
        ...
        stats_data = {
            ...
            "api_id_connected": api_id_connected,
            "api_hash_connected": api_hash_connected,
            "bot_token_connected": bot_token_connected,
            "started_at": start_time if 'start_time' in globals() else 0
        }
